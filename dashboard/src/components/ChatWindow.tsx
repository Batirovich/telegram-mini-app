        ...
        {canExtract && (
          <div className={`flex ${isAdmin ? 'justify-end' : 'justify-start'}`}>
            <button
              ...
              className="text-[11px] text-slate-500 hover:text-blue-400 transition-colors opacity-0 group-hover:opacity-100 flex items-center gap-1"
            >
              {msg.type === 'photo' ? '🔍 Analyze & Extract Order' : '📋 Extract as Order'}
            </button>
          </div>
        )}
      </div>
...
export default function ChatWindow({ conversationId, clientId, clientName, onExtractOrder }: Props) {
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0">
      {/* Header */}
      <div className="px-5 py-3.5 border-b border-slate-700 bg-slate-800 flex items-center gap-3">
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4">
        {messages.length === 0 && (
          <div className="text-center text-slate-500 text-sm mt-16">No messages yet.</div>
        )}
        {messages.map(msg => (
          <MessageBubble
            key={msg._id}
      ...
      <div className="p-4 border-t border-slate-700 bg-slate-800">
        <div className="flex gap-2 items-end">
          <textarea
            ...
            rows={2}
            className="flex-1 bg-slate-700 text-white placeholder-slate-500 rounded-xl px-4 py-3 text-sm resize-none outline-none focus:ring-2 focus:ring-blue-500 transition"
          />
          <button
            onClick={handleSend}
            disabled={!replyText.trim() || sending}
            className="bg-blue-600 hover:bg-blue-700 disabled:opacity-40 disabled:cursor-not-allowed text-white rounded-xl px-5 py-3 text-sm font-medium transition-colors h-[58px]"
          >
            {sending ? '...' : 'Send'}
          </button>
        </div>
